...
A data science internship project that explores, cleans, and models the [Kaggle Housing Prices Dataset](https://www.kaggle.com/datasets/yasserh/housing-prices-dataset) to predict house prices using Linear Regression and Random Forest Regressor.
...
1. **Data Loading & Exploration** — Load CSV, inspect shape, identify target/features, check missing values
2. **Data Cleaning** — Handle missing values, remove duplicates, one-hot encode categorical columns
3. **Model Building** — 80/20 train-test split, train Linear Regression and Random Forest, evaluate with MAE, RMSE, R²
4. **Visualization** — Price distribution histogram, correlation heatmap, actual vs predicted scatter plot
5. **Insights & Summary** — Key findings, model accuracy in plain terms, and a business recommendation
